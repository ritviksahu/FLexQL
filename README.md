...
Run the interactive REPL client in another terminal:
...
Run the dedicated select/query benchmark:

```bash
./build/benchmark_select
```
...
Run the select/query benchmark with custom parameters:

```bash
./build/benchmark_select 100000 1000 5 1000
```

Run the REPL client with a custom host and port:
...
```sql
CREATE TABLE T(ID DECIMAL, NAME VARCHAR(20));
...
Parameters for `benchmark_select` are:

- seed rows
- queries per set
- rounds
- insert batch size for seeding
...
The project also includes `benchmark_select` for focused query benchmarking:

- Point lookups using `WHERE ID = ...`
- Range scans using `WHERE ID >= ... AND ID <= ...`
- Join queries over `SEL_USERS` and `SEL_EVENTS`
- `COUNT(*)` queries over filtered ranges
